...
def extract_projects(text):
    # Improved regex to find all project sections
    project_pattern = re.compile(
        r"(?i)(?:Projects?|Academic Projects?):?\s*(.*?)(?=\n\n|\Z)", re.DOTALL
    )
    projects_sections = project_pattern.findall(text)
...
def extract_project_details(text):
    ...
    for project in projects:
        # Try to split project name and description
        parts = project.split(":", 1)
        if len(parts) > 1:
            name = parts[0].strip()
            description = parts[1].strip()
        else:
            # If no clear separation, treat the whole text as the name
            name = project.strip()
            description = ""

        project_details.append({"name": name, "description": description})

    return project_details


def extract_experience_details(text):
    # Improved regex to find experience section
    experience_pattern = re.compile(
        r"(?i)(?:Experience|Work Experience):?\s*(.*?)(?=(?:Education|Skills|Projects?)|$)",
        re.DOTALL,
    )
    ...
    if experience_section:
        ...
        # Split individual experiences
        individual_experiences = re.split(r"\n(?=[A-Z])", experience_text.strip())

        experiences = []
        for exp in individual_experiences:
            # Try to extract company and duration
            company_duration = re.match(r"(.*?)\s*(?:\((.*?)\))?", exp)
            if company_duration:
                company = company_duration.group(1).strip()
                duration = (
                    company_duration.group(2).strip()
                    if company_duration.group(2)
                    else ""
                )
                details = exp[len(company) + len(duration) :].strip()
                if duration:
                    details = details[len(duration) + 2 :].strip()  # +2 for parentheses

                experiences.append(
                    {"company": company, "duration": duration, "details": details}
                )
